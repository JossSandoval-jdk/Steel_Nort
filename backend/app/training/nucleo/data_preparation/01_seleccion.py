...
def main():
    ...
    variables_diagnostico = set(
        config.VARIABLES_DIAGNOSTICO
    )

    variables_redundantes = set(
        config.VARIABLES_REDUNDANTES
    )

    variables_correccion = set(
        config.VARIABLES_REQUIEREN_CORRECCION
    )

    variables_excluidas = set(
        config.EXCLUIR_MEDICION_CORRUPTA
    )
    ...
    for f in config.FEATURES_SELECCIONADAS:
        ...
        # Determinar clasificación de la variable
    ...
    log(
        f"Variables seleccionadas: {len(df)}"
    )
