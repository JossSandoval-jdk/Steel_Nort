...
# NOTA: config.py vive en backend/app/training/nucleo/data_preparation,
...
def descubrir_corridas(raiz=None):
    """Devuelve las corridas disponibles: subcarpetas de baseline/ y
    anomalias/ con metrics.log, más cualquier corrida legacy directa
    en la raiz."""
    raiz = Path(raiz) if raiz else OUTPUT_BASE_DIR

    def _con_metricas(carpeta):
        ...
        return [
            ...
            and os.path.isfile(
                os.path.join(carpeta, d, NOMBRE_METRICAS))
        ]

    corridas = _con_metricas(DIR_BASELINE) + _con_metricas(DIR_ANOMALIAS)

    for d in sorted(os.listdir(raiz)):
        p = raiz / d
        if (os.path.isdir(p)
                and d not in ("baseline", "anomalias")
                and os.path.isfile(p / NOMBRE_METRICAS)):
            corridas.append(d)

    return sorted(set(corridas))
