"""
03_deteccion_copod.py
=====================
Detector de anomalías usando COPOD (Copula-Based Outlier Detection).
Modelo basado en funciones de copula estadística, muy eficiente para datos multivariantes.
"""
...
def log(msg):
    print(f"[DETECCION COPOD] {msg}", flush=True)
...
def main():
    ...
    modelo = COPOD(contamination=0.05)
    modelo.fit(X_train)

    os.makedirs(config.DIR_DETECCION, exist_ok=True)
    joblib.dump(modelo, os.path.join(config.DIR_DETECCION, "modelo_copod.joblib"))
    log("Modelo COPOD entrenado y guardado (sin tocar scaler.joblib de producción).")

    # 2. Scores y Umbrales
    # En PyOD, decision_function da valores altos para anomalías. 
    # Lo invertimos (-1) para mantener el estándar scikit-learn (menor score = más anomalía).
    ...
    # PyOD predict devuelve 1 para anomalía y 0 para normal. 
    # Se mapea a -1 (anomalía) y 1 (normal) para compatibilidad con el resto del pipeline.
    pred_test_mapped = np.where(modelo.predict(X_test) == 1, -1, 1)
    pred_train_mapped = np.where(modelo.predict(X_train) == 1, -1, 1)
    ...
    df_alertas = pd.DataFrame({
        ...
        "score": scores_test,
        "prediccion": pred_test_mapped,
    })

    for nombre, umbral in umbrales.items():
        df_alertas[nombre] = (df_alertas["score"] < umbral).astype(int)

    df_alertas.to_csv(os.path.join(config.DIR_DETECCION, "alertas_test_copod.csv"), index=False, encoding="utf-8-sig")
    ...
    df_scores = pd.DataFrame({
        ...
        "run": train["runs"],
        "score": scores_train,
        "prediccion": pred_train_mapped,
    })
    df_scores.to_csv(os.path.join(config.DIR_DETECCION, "scores_muestras_copod.csv"), index=False, encoding="utf-8-sig")

    # 5. Importancia de variables (Permutation Importance adaptada con score invertido)
    ...
    df_imp.to_csv(os.path.join(config.DIR_DETECCION, "importancia_variables_copod.csv"), index=False, encoding="utf-8-sig")

    # 6. Resumen por consola
    print("\n=== COPOD SOBRE REFERENCIA NORMAL ===")
