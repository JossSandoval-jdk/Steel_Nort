"""
03_deteccion_elliptic_envelope.py
=================================
Detector de anomalías usando Elliptic Envelope.
Asume que los datos normales siguen una distribución gaussiana multivariante 
y traza una frontera elíptica robusta.
"""
...
def log(msg):
    print(f"[DETECCION ELLIPTIC] {msg}", flush=True)
...
def main():
    ...
    # contamination define la proporción esperada de outliers en los datos
    modelo = EllipticEnvelope(random_state=config.SEED, contamination=0.05)
    modelo.fit(X_train)
    ...
    # un valor menor indica mayor grado de anomalía.
    ...
    df_alertas.to_csv(os.path.join(config.DIR_DETECCION, "alertas_test_elliptic.csv"), index=False, encoding="utf-8-sig")
    ...
    df_scores.to_csv(os.path.join(config.DIR_DETECCION, "scores_muestras_elliptic.csv"), index=False, encoding="utf-8-sig")
    ...
    df_imp.to_csv(os.path.join(config.DIR_DETECCION, "importancia_variables_elliptic.csv"), index=False, encoding="utf-8-sig")

    # 6. Resumen por consola
    print("\n=== ELLIPTIC ENVELOPE SOBRE REFERENCIA NORMAL ===")
